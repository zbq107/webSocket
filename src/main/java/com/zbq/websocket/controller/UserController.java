...
import com.zbq.websocket.util.CookieUtil;
...
public class UserController {
    ...
    public Result LoginPD(UserEntity user, HttpServletRequest request, HttpServletResponse response) {
        ...
        if(userIn==null) {
            ...
            try {
                //保证一个地方只有一个用户登录，先删除，后添加
                redisUtil.remove(userIn.getUserName());
    ...
    //注册方法
    @PostMapping("reg")
    public Result reg(UserEntity user) {
        Result result = new Result();
        //先查重，用户名是否存在
        UserEntity userIn = userService.selectUserName(user);
        //如果没查到就插入
        if(userIn == null) {
